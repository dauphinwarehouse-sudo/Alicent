...
5. Привязать одобрение к задаче, конкретному вызову (call ID), инструменту, его версии и хешу payload; одобрение расходуется один раз; нельзя использовать общий boolean от модели.
...
Политика инструмента задаёт три вида: `read_only`, `reversible` и `destructive`. Она же решает, насколько промпт должен принадлежать пользователю: смешанный промпт (инструкция пользователя плюс текст документа) допускает только `read_only` с явным `accepts_mixed_prompt`, `reversible` и `destructive` требуют промпта целиком от пользователя, а промпт без пользовательской инструкции запрещён для всех. Registry не даёт `read_only` инструменту запрашивать write- или network-permissions, поэтому ярлыком нельзя воспользоваться в обход проверки провенанса. Injection-сигналы запрещают вызов при любой политике.
...
`enqueue`, `cancel`, `resume`; durable task содержит budget (steps/deadline/cost) и lifecycle. Domain валидирует допустимые переходы и запрещает незаметный restart terminal-задач; задача, сломавшаяся во время ожидания подтверждения, переходит в `Failed` напрямую, без ложной записи об исполнении. Планируемое исполнение: очередь → планирование → подтверждение при необходимости → шаги → завершение. Pause/resume должны восстанавливать лишь подтверждённый tool ledger, не повторять side effects.
