...
Просмотрены стартовый экран и рабочая область 1440×940, светлая/тёмная тема, диалог создания, diff/restore и мобильная компоновка 390 px. Проверены читаемость, отсутствие горизонтального overflow и видимость истории на узком экране. Снимки рабочей области сделаны с отдельным тестовым портом, не с native backend. Полный accessibility audit ещё не выполнен.

## Не подтверждено

- Native Rust/Tauri сборка и установленное приложение Windows 10/11. Шаблон CI подготовлен, но не активирован: GitHub token отклонил запись workflow. Успешного CI run нет. Установщик здесь не заявлен готовым.
