...
test("preview accurately disables native project access", async ({ page }) => {
  await page.goto("/");
  await expect(page.getByText("Предпросмотр интерфейса")).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Новый проект" }),
  ).toBeDisabled();
});
test("edit, autosave, history diff and restore update the actual editor", async ({
  page,
}) => {
...
test("new scene, folder navigation and literal search", async ({ page }) => {
  ...
  await page.getByLabel("Название", { exact: true }).fill("Вторая сцена");
  await page.getByRole("button", { name: "Создать", exact: true }).click();
  await page
    .getByRole("textbox", { name: "Текст документа" })
    .fill("Ключевое слово: маяк.");
  await page.getByRole("textbox", { name: "Поиск по проекту" }).fill("маяк");
  await page.getByRole("button", { name: "Найти", exact: true }).click();
  await expect(
    page.getByRole("navigation", { name: "Документы" }),
  ).toContainText("Вторая сцена");
  await page.getByRole("button", { name: "+ Папка", exact: true }).click();
  await page.getByLabel("Название", { exact: true }).fill("Часть II");
  await page.getByRole("button", { name: "Создать", exact: true }).click();
