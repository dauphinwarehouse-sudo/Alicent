...
impl ApprovalEngine {
    ...
    fn prune(&mut self, now_epoch_secs: u64) {
        let grace = APPROVAL_RETENTION_GRACE_SECS;
        self.approvals.retain(|_, record| {
            let retain_until = record.expires_at.saturating_add(grace);
            now_epoch_secs <= retain_until
        });
    }

    /// Keeps the tracked set bounded by evicting the record closest to expiry.
    /// Aged-out records are already gone by the time this runs, so it only
    /// triggers for an implausible number of live approvals.
    fn enforce_capacity(&mut self) {
        while self.approvals.len() >= MAX_TRACKED_APPROVALS {
            let mut oldest: Option<(String, u64)> = None;
            for (id, record) in &self.approvals {
                let is_earlier = match &oldest {
                    Some((_, expires_at)) => record.expires_at < *expires_at,
                    None => true,
                };
                if is_earlier {
                    oldest = Some((id.clone(), record.expires_at));
                }
            }
            match oldest {
                Some((id, _)) => {
