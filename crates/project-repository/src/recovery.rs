...
pub(super) fn validate_database(conn: &Connection) -> Result<i64> {
    ...
    if version >= 2 { expected.execute_batch(include_str!("schema-v2.sql"))?; }
    if version >= 3 { expected.execute_batch(include_str!("schema-v3.sql"))?; }
    if version >= 4 { expected.execute_batch(include_str!("schema-v4.sql"))?; }
    if schema(conn)? != schema(&expected)? { return Err(Error::Integrity); }
    let check: String = conn.query_row("PRAGMA quick_check", [], |r| r.get(0))?;
    if check != "ok" { return Err(Error::Integrity); }
    if conn.prepare("PRAGMA foreign_key_check")?.query([])?.next()?.is_some() { return Err(Error::Integrity); }
    let (count, stored): (i64, Option<i64>) = conn.query_row("SELECT count(*),min(schema_version) FROM project", [], |r| Ok((r.get(0)?, r.get(1)?)))?;
    if count != 1 || stored != Some(version) { return Err(Error::Integrity); }
    ...
    if inconsistent != 0 { return Err(Error::Integrity); }
    if version >= 3 {
        ...
        if invalid_archive != 0 { return Err(Error::Integrity); }
    }
    if version >= 4 {
        ...
        if invalid_order != 0 { return Err(Error::Integrity); }
    }
    Ok(version)
}

fn copy_database(source: &Connection, parent: &Path, name: &str, cancelled: &AtomicBool) -> Result<BackupInfo> {
    ensure_plain_path(parent)?;
    if cancelled.load(Ordering::Relaxed) { return Err(Error::Cancelled); }
    let parent = parent.canonicalize()?;
    let temporary = tempfile::Builder::new().prefix(".alicent-backup-").tempfile_in(&parent)?;
    ...
    let started = Instant::now();
    {
        ...
        loop {
            if cancelled.load(Ordering::Relaxed) { return Err(Error::Cancelled); }
            if started.elapsed() > Duration::from_secs(300) { return Err(Error::TimedOut); }
    ...
    Ok(BackupInfo { path: target.to_string_lossy().into_owned(), bytes, schema_version: version })
}

fn migrate(conn: &mut Connection, root: &Path, from: i64, script: &str) -> Result<()> {
    let backups = root.join("backups");
    if !backups.exists() { fs::create_dir(&backups)?; }
    ensure_plain_path(&backups)?;
    let tx = conn.transaction()?;
    let version: i64 = tx.query_row("PRAGMA user_version", [], |r| r.get(0))?;
    if version != from { return Err(Error::UnsupportedSchema); }
    copy_database(&tx, &backups, &format!("pre-migration-v{from}-{}.alicent-backup", Uuid::new_v4()), &AtomicBool::new(false))?;
    tx.execute_batch(script)?;
    tx.commit()?;
    Ok(())
}

pub(super) fn migrate_v1(conn: &mut Connection, root: &Path) -> Result<()> { migrate(conn, root, 1, include_str!("schema-v2.sql")) }
pub(super) fn migrate_v2(conn: &mut Connection, root: &Path) -> Result<()> { migrate(conn, root, 2, include_str!("schema-v3.sql")) }
pub(super) fn migrate_v3(conn: &mut Connection, root: &Path) -> Result<()> { migrate(conn, root, 3, include_str!("schema-v4.sql")) }

impl Repository {
    pub fn backup_to(&self, directory: &Path, cancelled: &AtomicBool) -> Result<BackupInfo> {
        copy_database(&self.conn, directory, &format!("{}-{}.alicent-backup", self.project()?.id, Uuid::new_v4()), cancelled)
    }
    pub fn restore_backup(backup: &Path, parent: &Path, cancelled: &AtomicBool) -> Result<Self> {
        ...
        let source = Connection::open_with_flags(backup, OpenFlags::SQLITE_OPEN_READ_ONLY | OpenFlags::SQLITE_OPEN_NO_MUTEX)?;
        source.execute_batch("PRAGMA trusted_schema=OFF; BEGIN;")?;
        validate_database(&source)?;
        let parent = parent.canonicalize()?;
        let staging = tempfile::Builder::new().prefix(".alicent-restore-").tempdir_in(&parent)?;
        ...
        if fs::symlink_metadata(&root).is_ok() { return Err(Error::UnsafePath); }
