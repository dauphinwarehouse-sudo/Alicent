...
fn live_backup_is_standalone_and_restore_never_replaces_original() {
    ...
    assert_eq!(
        imported.read(doc.summary.id).unwrap().content,
        first.content
    );
...
fn cancelled_backup_and_restore_publish_nothing() {
    ...
    let target = TempDir::new().unwrap();
    assert!(matches!(
        repo.backup_to(target.path(), &AtomicBool::new(true)),
        Err(Error::Cancelled)
    ));
...
fn migration_creates_readable_v1_backup_before_upgrading() {
    ...
    assert_eq!(
        old.query_row("SELECT content FROM documents", [], |r| r
            .get::<_, String>(0))
            .unwrap(),
        "Прежний текст"
    );
...
#[test]
fn migration_from_v2_creates_strict_readable_backups_through_v4() {
    let dir = TempDir::new().unwrap();
...
fn failure_to_create_pre_migration_backup_leaves_schema_v1() {
    ...
    assert_eq!(
        db.query_row("SELECT content FROM documents", [], |r| r
            .get::<_, String>(0))
            .unwrap(),
        "Прежний текст"
    );
...
fn imported_unknown_schema_and_triggers_are_rejected_without_touching_source() {
    ...
    db.execute_batch(
        "CREATE TRIGGER unexpected AFTER UPDATE ON documents BEGIN DELETE FROM versions; END;",
    )
    .unwrap();
    drop(db);
...
fn checkpoint_restores_all_texts_atomically_and_preserves_new_documents() {
    ...
    let b = repo
        .create_document("Вторая", DocumentKind::Note, None)
        .unwrap();
    let b = save(&mut repo, &b, "Маяк");
    let cp = repo
        .create_checkpoint(Uuid::new_v4(), "Первый вариант")
        .unwrap();
    assert_eq!(cp.document_count, 2);
    let a = save(&mut repo, &a, "Юг");
    let b = save(&mut repo, &b, "Город");
    let later = repo
        .create_document("После снимка", DocumentKind::Scene, None)
        .unwrap();
    ...
    let result = repo
        .restore_checkpoint(cp.id, preview.project_revision, command)
        .unwrap();
    ...
    assert_eq!(
        repo.version_content(a.summary.id, a.summary.revision)
            .unwrap(),
        "Юг"
    );
    ...
    assert_eq!(
        repo.read(a.summary.id).unwrap().summary.revision,
        a.summary.revision + 1
    );
}
#[test]
fn concurrent_project_change_invalidates_checkpoint_preview() {
    ...
    assert!(matches!(
        repo.restore_checkpoint(cp.id, preview.project_revision, Uuid::new_v4()),
        Err(Error::ProjectConflict)
    ));
    assert_eq!(
        repo.read(doc.summary.id).unwrap().content,
        "Правка другого окна"
    );
}
#[test]
fn checkpoint_creation_is_idempotent_and_restore_noop_has_no_new_versions() {
    ...
    assert!(matches!(
        repo.create_checkpoint(id, "Другой"),
        Err(Error::CommandMismatch)
    ));
    let preview = repo.checkpoint_preview(id, 20, 0).unwrap();
    let cmd = Uuid::new_v4();
    assert_eq!(
        repo.restore_checkpoint(id, preview.project_revision, cmd)
            .unwrap()
            .changed_count,
        0
    );
    ...
    assert!(matches!(
        repo.restore_checkpoint(id, 999, cmd),
        Err(Error::CommandMismatch)
    ));
}
#[test]
fn mid_restore_failure_rolls_back_every_document_version_and_index() {
    let (_dir, mut repo, a) = setup();
    let b = repo
        .create_document("Вторая", DocumentKind::Scene, None)
        .unwrap();
    ...
    // Fault injection only after a valid repository is opened: second update aborts the transaction.
    ...
    assert!(repo
        .restore_checkpoint(cp.id, preview.project_revision, Uuid::new_v4())
        .is_err());
...
fn crash_during_schema_transaction_recovers_then_migrates_cleanly() {
    ...
    let exit = std::process::Command::new(std::env::current_exe().unwrap())
        .args(["--exact", "migration_crash_child", "--nocapture"])
        .env("ALICENT_MIGRATION_CRASH", root.join("project.sqlite3"))
        .status()
        .unwrap();
...
fn symlink_backup_directory_is_rejected() {
    ...
    assert!(matches!(
        repo.backup_to(&link, &AtomicBool::new(false)),
...
fn automatic_undo_checkpoint_restores_the_pre_rollback_texts() {
    ...
    let cp = repo
        .create_checkpoint(Uuid::new_v4(), "Первая редакция")
        .unwrap();
    ...
    repo.restore_checkpoint(undo_id, preview.project_revision, Uuid::new_v4())
        .unwrap();
    assert_eq!(
        repo.read(doc.summary.id).unwrap().content,
        "Вторая редакция"
...
fn cancelled_checkpoint_restore_keeps_text_and_publishes_no_undo_point() {
    ...
    let preview = repo.checkpoint_preview(cp.id, 200, 0).unwrap();
    let result = repo.restore_checkpoint_cancellable(
        cp.id,
...
fn checkpoint_preview_and_list_are_paginated_without_loading_texts() {
    ...
    for i in 0..4 {
        repo.create_document(&format!("Сцена {i}"), DocumentKind::Scene, None)
            .unwrap();
    }
    ...
    assert!(matches!(
        repo.checkpoint_preview(cp.id, 201, 0),
