...
fn create_crash_child() {
    let Ok(path) = std::env::var("ALICENT_CREATE_CRASH_DB") else { return; };
    ...
    conn.execute("INSERT INTO documents(id,title,kind,order_key) VALUES(?1,'Незавершённая','scene',1024)", [&document_id]).unwrap();
    conn.execute("INSERT INTO versions(document_id,revision,content,actor) VALUES(?1,0,'','user:local')", [&document_id]).unwrap();
    ...
    conn.execute("INSERT INTO receipts(command_id,payload_hash,result) VALUES(?1,'pending','{}')", [operation_id]).unwrap();
    std::process::exit(23);
}
...
fn crash_before_create_commit_leaves_no_partial_rows_and_retry_is_safe() {
    ...
    let created = repo.create_document_with_operation_id(operation_id, "Завершённая", DocumentKind::Scene, None).unwrap();
    drop(repo);
    let mut repo = Repository::open(&root).unwrap();
    let replay = repo.create_document_with_operation_id(operation_id, "Завершённая", DocumentKind::Scene, None).unwrap();
