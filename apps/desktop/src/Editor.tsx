...
export function Editor({ initial, onChange, readOnly = false }: { initial: string; onChange: (text: string) => void; readOnly?: boolean }) {
  ...
  useEffect(() => {
    ...
    const view = new EditorView({ parent: host.current, state: EditorState.create({ doc: initial, extensions: [
      ...
      syntaxHighlighting(defaultHighlightStyle), EditorView.lineWrapping,
      EditorView.contentAttributes.of({ 'aria-label': 'Текст документа', spellcheck: 'true', lang: 'ru' }),
