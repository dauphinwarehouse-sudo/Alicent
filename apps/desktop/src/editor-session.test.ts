...
describe("serialized autosave", () => {
  it("debounces edits and commits only the latest text", async () => {
    vi.useFakeTimers();
    const save = vi.fn(async (cmd: SaveDocument) => saved(cmd));
    const session = new EditorSession(doc, { save }, vi.fn());
    session.edit("Один");
    session.edit("Два");
    await vi.advanceTimersByTimeAsync(650);
    expect(save).toHaveBeenCalledTimes(1);
    expect(save.mock.calls[0][0].content).toBe("Два");
  ...
  it("serializes edits arriving during an in-flight save", async () => {
    ...
    const save = vi
      .fn<(cmd: SaveDocument) => Promise<Document>>()
    ...
    expect(save).toHaveBeenCalledTimes(1);
    resolve(saved(save.mock.calls[0][0]));
    await first;
    expect(save).toHaveBeenCalledTimes(2);
    expect(save.mock.calls[1][0].expected_revision).toBe(1);
    expect(session.document.content).toBe("Вторая");
  ...
  it("retains a failed draft, rejects navigation flush, supports retry", async () => {
    const save = vi
      .fn<(cmd: SaveDocument) => Promise<Document>>()
      .mockRejectedValueOnce("Диск заполнен")
      .mockImplementation(async (cmd) => saved(cmd));
    const session = new EditorSession(doc, { save }, vi.fn());
    session.edit("Не потерять");
    await expect(session.flush()).rejects.toBe("Диск заполнен");
    expect(session.content).toBe("Не потерять");
    expect(session.document.content).toBe("Начало");
    expect(session.state).toBe("error");
    expect(session.dirty).toBe(true);
    await session.flush();
    expect(session.document.content).toBe("Не потерять");
    session.dispose();
  });
...
it("reuses command identity after a lost acknowledgement before saving newer edits", async () => {
  const save = vi
    .fn<(cmd: SaveDocument) => Promise<Document>>()
    .mockRejectedValueOnce("Ответ потерян")
    .mockImplementation(async (cmd) => saved(cmd));
  const session = new EditorSession(doc, { save }, vi.fn());
  session.edit("Первая");
  await expect(session.flush()).rejects.toBe("Ответ потерян");
  session.edit("Вторая");
  await session.flush();
