...
export const api: ProjectPort = {
  ...
  createDocument: (title, kind, parent) => invoke('create_document', { title, kind, parent }),
  ...
  restore: (id, revision, expectedRevision, commandId) => invoke('restore_version', { id, revision, expectedRevision, commandId }),
};
